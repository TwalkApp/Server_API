...
func GetUserFollows(id string, pagination misc.Pagination) ([]users.Profile, error) {
	var (
		user	users.Profile
		result	[]users.Profile
	)
	result = make([]users.Profile, 0)
	query := "SELECT id, username, mail, firstname, lastname FROM users WHERE id IN (SELECT follow_id FROM subscriptions WHERE user_id = ?)"
	if pagination.IsSet() {
